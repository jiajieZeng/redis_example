...
import (
	...
	// "redis_course/example/common"
)


const ex04ListenList = "ex04_list_0"
...
func Ex04(ctx context.Context) {
	...
	// new一个并发执行器
	// routineNums是消费端的数量，多消费场景，可以使用ex04ConsumerPop,使用ex04ConsumerRange存在消息重复消费的问题。
...
// ex04ConsumerRange 使用lrang批量消费队列中的数据，数据保留在队列中
// 生成断使用: rpush ex04_list_0 AA BB
// 消费端:
// 方法1 lrange ex04_list_0 -3 -1 
// 从FIFO队尾中一次消费3条信息
...
func ex04ConsumerRange(ctx context.Context, cInstParam common.CInstParams) {
	...
	for {
		// 从index(-consumeBatchSize)开始取，知道最后一个元素index(-1)
